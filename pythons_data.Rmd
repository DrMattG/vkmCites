...
##Fishing out python timoriensis data for comparison with CITES trade database
```{r}
pt <- vkmCites::Pythons |>
  dplyr::filter(Year > 2009, Taxon =="Python timoriensis")
summary(pt)
write.csv(pt, "pt.csv", row.names=F)

preti <- vkmCites::Pythons |>
  dplyr::filter(Year > 2009, Taxon =="Python reticulatus")
summary(preti)
write.csv(preti, "preti.csv", row.names=F)
```
